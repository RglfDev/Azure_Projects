 ...
 */
class RestApiClient {
    constructor(config, userAgent, httpRequestBuilder) {
    ...
    executeApiCall(method, path, headers, requestBody, timeout, requestOptions, done) {
        ...
        if (this._config.tokenCredential) {
            ...
            if (this._config.sharedAccessSignature) {
                httpHeaders.Authorization = (typeof (this._config.sharedAccessSignature) === 'string') ? this._config.sharedAccessSignature : this._config.sharedAccessSignature.extend((0, azure_iot_common_1.anHourFromNow)());
            }
            this.executeBody(requestBody, httpHeaders, headers, method, path, timeout, requestOptions, done);
        }
    }
     ...
     */
    isAccessTokenCloseToExpiry(accessToken) {
    ...
    async getToken() {
        if ((!this._accessToken) || this.isAccessTokenCloseToExpiry(this._accessToken)) {
            this._accessToken = await this._config.tokenCredential.getToken(this._config.tokenScope);
        }
    ...
    executeBody(requestBody, httpHeaders, headers, method, path, timeout, requestOptions, done) {
        httpHeaders['Request-Id'] = (0, node_crypto_1.randomUUID)();
    ...
    static translateError(body, response) {
        ...
        let error;
        const errorContent = http_1.Http.parseErrorBody(body);
